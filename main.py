...
def late_func(text,func,znach,edinic = "",precis1 = 4, precis2 = 2, prived = False):
    eqq = text
    znach = list(znach.items())
    zqq = eqq
    mass = {}
    for j in znach:
        eqq = eqq.replace(j[0],numpy.format_float_positional(j[1],precision=precis1,trim='-'))
    ...
    if prived:
        ...
        resull = rr[0]
        if rr[1] == 0:
            vivod = numpy.format_float_positional(resull, precision=precis2, trim='-')
        else:
            vivod = numpy.format_float_positional(resull,precision=2,trim='-') + r"\cdot 10^{  "+str(rr[1])+"  }"
    else:
        vivod = numpy.format_float_positional(resul, precision=precis2, trim='-')
...
def raschet(E,Rn,Cn,Kooc,Mn,Mv,fn,fv,Uvix,kpy,typt):
    ...
    Ivixl = late_func(r" \frac {U_{вых} } {R_{н} } ", "I_{вых} = ", {"U_{вых}": Uvix,"R_{н}": Rn/1000 }, "мА",precis1=6, precis2=6)
    El = "E_{пит} = " + str(E) + r"\mathrm{В}"
    Pl = late_func(r"\frac {U_{вых}^{2} } {2 \cdot R_{н} }", "P_{вых} = ",{"U_{вых}": Uvix,"R_{н}": Rn/1000 }, "мВт",precis1=6, precis2=6)
    PPl = late_func(r"3 \cdot P_{вых}", "P_{pac} = ", {"P_{вых}":Pl[1]}, "мВт", precis1=6, precis2=6)
    Fgrl = late_func(r"3 \cdot f_{в} " , "f_{гр} = ", { "f_{в}": fv}, "Гц", precis1=6, prived=True)
    ...
    tr = tranz3102(typt,Ivixl[1]*1.5,Uke0,Uvix,E)
    yy = tr.yparam()
    tr.drawWork(kpy)
    ypar = yy[1]
    y11 = ypar[0]
    ...
    Re = late_func(r"\frac {0.2 \cdot E} {(I_{k0} + I_{b0})}","R_{e} = ",{"E": E, "I_{k0}":Ik0/pow(10,3), "I_{b0}":Ib0/pow(10,6)},"Ом",precis1= 9)
    Re[1] = nomin(Re[1])
    Rk = late_func(r"\frac {E - (I_{k0} + I_{b0}) \cdot R_{e} - U_{ke0}} {I_{k0}}","R_{k} = ",{"E":E,"U_{ke0}":Uk0,"I_{k0}":Ik0/pow(10,3),"R_{e}": Re[1], "I_{b0}":Ib0/pow(10,6)},"Ом",precis1= 9)
    Rk[1] = nomin(Rk[1])
    ...
    an = late_func(r" \sqrt{(\frac {1} { M_{ср} })^2 - 1  } ", "a_{н} = ", {"M_{ср}":Mnp},precis2=5)
    ...
    Ye = late_func(r" Y_{22} + Y_{k} + Y_{н} +  ( 2 \cdot \pi \cdot f_{н} \cdot C_{н} ) ", "Y_{екв} = ", { "\pi": 3.1415 , "f_{н}":fn, "Y_{22}": y22[1]/pow(10,3),"Y_{k}": 1/Rk[1], "Y_{н}": 1/Rn, "C_{н}": Cn}, "См",precis2= 6 )
    av = late_func(r"\frac {2 \cdot \pi \cdot f_{в} \cdot C_{0}} { Y_{екв} }" , "a_{в} = " , { "\pi": 3.1415 , "f_{в}":fv, "C_{0}": C0[1]/pow(10,12), "Y_{екв}": Ye[1]}, precis2=5, precis1=15 )
    Mv = late_func(r"\frac {1} {\sqrt{ a_{в}^2 + 1 }}", "M_{в}", { "a_{в}": av[1]},precis2=3 )
    Uvx = late_func(r"\frac {U_{вых}} {K_{ooc}}", "U_{вх} = ", {"U_{вых}": Uvix, "K_{ooc}": Kooc},"В", precis2=6)
...
if __name__ == '__main__':
    ...
    fv = 30000
    Uvix = 3.5
    vv = raschet(E,Rn,Cn,Kooc,Mn,Mv,fn,fv,Uvix,1,1)
    E = E*0.7
    for i in range(2,kpu+1):
        vv = raschet(E,vv[0],Cn,Kooc,Mn,Mv,fn,fv,vv[1],i,3)
